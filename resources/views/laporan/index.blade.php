...
@section('content')
    <div class="space-y-4 sm:space-y-6">
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div class="min-w-0 flex-1 mr-2">
                        ...
                        <h3 class="text-xl sm:text-2xl lg:text-3xl font-bold truncate">Rp
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div class="min-w-0 flex-1 mr-2">
                        ...
                        <h3 class="text-xl sm:text-2xl lg:text-3xl font-bold truncate">Rp
                            {{ number_format($totalKembalian, 0, ',', '.') }}</h3>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6">
                ...
                @php
                    $perPageTerlaris = 5;
                    $pageTerlaris = (int) request('page_terlaris', 1);
                    $slicedTerlaris = $produkTerlaris->slice(($pageTerlaris - 1) * $perPageTerlaris, $perPageTerlaris);
                    $totalPageTerlaris = (int) ceil($produkTerlaris->count() / $perPageTerlaris);
                @endphp

                {{-- Mobile cards --}}
                <div class="block lg:hidden space-y-3">
                    @forelse ($slicedTerlaris as $produk)
                        <div class="bg-gradient-to-r from-gray-50 to-white border border-gray-200 rounded-lg p-3">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span
                                    class="ml-2 bg-blue-100 text-blue-800 px-2.5 py-1 rounded-full text-xs font-bold whitespace-nowrap">{{ $produk->total_qty }}x</span>
                            </div>
                            <div class="text-right">
                                <span class="text-base font-bold text-emerald-600">Rp
                                    {{ number_format($produk->total_penjualan, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    @empty
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($slicedTerlaris as $produk)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 text-sm font-semibold text-gray-900">{{ $produk->nama_produk }}
                                    </td>
                                    <td class="px-4 py-3 text-sm text-center">
                                        <span
                                            class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full font-bold text-xs">{{ $produk->total_qty }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-right font-bold">Rp
                                        {{ number_format($produk->total_penjualan, 0, ',', '.') }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="px-4 py-8 text-center text-gray-500 text-sm">Tidak ada data
                                        produk</td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                @if ($totalPageTerlaris > 1)
                    <div class="flex items-center justify-between mt-4 pt-3 border-t border-gray-200">
                        <span class="text-xs text-gray-500">
                            Hal {{ $pageTerlaris }} / {{ $totalPageTerlaris }}
                            &bull; {{ $produkTerlaris->count() }} produk
                        </span>
                        <div class="flex items-center gap-1">
                            @if ($pageTerlaris > 1)
                                <a href="{{ request()->fullUrlWithQuery(['page_terlaris' => $pageTerlaris - 1]) }}#produk-terlaris"
                                    class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-semibold transition">
                                    <i class="fas fa-chevron-left mr-1"></i>Prev
                                </a>
                            @endif

                            @for ($p = max(1, $pageTerlaris - 1); $p <= min($totalPageTerlaris, $pageTerlaris + 1); $p++)
                                <a href="{{ request()->fullUrlWithQuery(['page_terlaris' => $p]) }}#produk-terlaris"
                                    class="px-3 py-1.5 rounded-lg text-xs font-semibold transition {{ $p == $pageTerlaris ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700' }}">
                                    {{ $p }}
                                </a>
                            @endfor

                            @if ($pageTerlaris < $totalPageTerlaris)
                                <a href="{{ request()->fullUrlWithQuery(['page_terlaris' => $pageTerlaris + 1]) }}#produk-terlaris"
                                    class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-semibold transition">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6">
                ...
                @php
                    $perPageKasir = 5;
                    $pageKasir = (int) request('page_kasir', 1);
                    $slicedKasir = $laporanPerKasir->slice(($pageKasir - 1) * $perPageKasir, $perPageKasir);
                    $totalPageKasir = (int) ceil($laporanPerKasir->count() / $perPageKasir);
                @endphp

                {{-- Mobile cards --}}
                <div class="block lg:hidden space-y-3">
                    @forelse ($slicedKasir as $kasir)
                        <div class="bg-gradient-to-r from-gray-50 to-white border border-gray-200 rounded-lg p-3">
                            <div class="flex items-center justify-between mb-2">
                                <span
                                    class="font-semibold text-gray-900 text-sm flex-1 truncate">{{ $kasir->nama_kasir }}</span>
                                <span
                                    class="ml-2 bg-green-100 text-green-800 px-2.5 py-1 rounded-full text-xs font-bold whitespace-nowrap">{{ $kasir->total_transaksi }}x</span>
                            </div>
                            <div class="text-right">
                                <span class="text-base font-bold text-emerald-600">Rp
                                    {{ number_format($kasir->total_penjualan, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    @empty
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($slicedKasir as $kasir)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-sm text-center">
                                        <span
                                            class="bg-green-100 text-green-800 px-3 py-1 rounded-full font-bold text-xs">{{ $kasir->total_transaksi }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-right font-bold">Rp
                ...
                {{-- Pagination Per Kasir --}}
                @if ($totalPageKasir > 1)
                    <div class="flex items-center justify-between mt-4 pt-3 border-t border-gray-200">
                        <span class="text-xs text-gray-500">
                            Hal {{ $pageKasir }} / {{ $totalPageKasir }}
                            &bull; {{ $laporanPerKasir->count() }} kasir
                        </span>
                        <div class="flex items-center gap-1">
                            @if ($pageKasir > 1)
                                <a href="{{ request()->fullUrlWithQuery(['page_kasir' => $pageKasir - 1]) }}#laporan-kasir"
                                    class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-semibold transition">
                                    <i class="fas fa-chevron-left mr-1"></i>Prev
                                </a>
                            @endif

                            @for ($p = max(1, $pageKasir - 1); $p <= min($totalPageKasir, $pageKasir + 1); $p++)
                                <a href="{{ request()->fullUrlWithQuery(['page_kasir' => $p]) }}#laporan-kasir"
                                    class="px-3 py-1.5 rounded-lg text-xs font-semibold transition {{ $p == $pageKasir ? 'bg-green-600 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700' }}">
                                    {{ $p }}
                                </a>
                            @endfor

                            @if ($pageKasir < $totalPageKasir)
                                <a href="{{ request()->fullUrlWithQuery(['page_kasir' => $pageKasir + 1]) }}#laporan-kasir"
                                    class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-semibold transition">
                                    Next<i class="fas fa-chevron-right ml-1"></i>
                                </a>
        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
                ...
                <button onclick="exportTableToExcel('tabelProduk', 'Laporan_Produk_Terjual')"
                    class="px-3 py-2 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-lg text-xs font-semibold transition-all">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                ...
                <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-3 border border-green-200">
                    ...
                    <p class="text-lg sm:text-xl font-bold text-green-700">
                        {{ number_format($detailProduk->sum('total_qty'), 0, ',', '.') }}</p>
                </div>
                <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-3 border border-purple-200">
                    ...
                    <p class="text-base sm:text-lg font-bold text-purple-700">Rp
                        {{ number_format($detailProduk->sum('total_penjualan'), 0, ',', '.') }}</p>
                </div>
                <div class="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-3 border border-orange-200">
                    ...
                    <p class="text-base sm:text-lg font-bold text-orange-700">
                        Rp
                        {{ $detailProduk->count() > 0 ? number_format($detailProduk->sum('total_penjualan') / $detailProduk->count(), 0, ',', '.') : 0 }}
                    </p>
                </div>
            </div>
            ...
            <div class="mb-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                <div class="relative">
                    ...
                    <input type="text" id="searchProduk"
                        class="w-full pl-11 pr-4 py-2.5 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm"
                        placeholder="Cari produk...">
                </div>
                <div class="relative">
                    <i class="fas fa-filter absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                    <select id="filterKategori"
                        class="w-full pl-11 pr-4 py-2.5 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm appearance-none bg-white">
                ...
                <table class="min-w-full divide-y divide-gray-200" id="tabelProduk">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($detailProduk as $index => $produk)
                            <tr class="produk-row hover:bg-indigo-50 transition-colors"
                                data-kategori="{{ $produk->id_produk_kategori ?? '' }}">
                                <td class="px-4 py-3 text-center">
                                    <button onclick="toggleDetail({{ $index }})"
                                        class="w-6 h-6 rounded-full bg-indigo-100 hover:bg-indigo-200 text-indigo-600 flex items-center justify-center transition-all">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <span
                                        class="font-mono text-xs bg-gray-100 px-2 py-1 rounded">{{ $produk->code_produk }}</span>
                                </td>
                                <td class="px-4 py-3 text-sm">
                                ...
                                <td class="px-4 py-3 text-sm text-center">
                                    <span
                                        class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full font-bold">{{ number_format($produk->total_qty, 0, ',', '.') }}</span>
                                </td>
                                <td class="px-4 py-3 text-sm text-center">
                                    <span
                                        class="bg-green-100 text-green-800 px-3 py-1 rounded-full font-bold">{{ $produk->total_transaksi }}</span>
                                </td>
                                <td class="px-4 py-3 text-sm text-right font-medium text-gray-700">Rp
                                    {{ number_format($produk->harga_satuan, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-sm text-right font-bold text-indigo-600">Rp
                                    {{ number_format($produk->total_penjualan, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-sm text-center text-gray-600">
                                    {{ number_format($produk->rata_rata_qty, 1) }}</td>
                            </tr>
                                ...
                                <td colspan="9" class="px-4 py-4">
                                    <div class="bg-white rounded-lg shadow-sm p-4">
                                        <h4 class="text-sm font-bold text-gray-800 mb-3 flex items-center">
                                            ...
                                            Detail Transaksi - {{ $produk->nama_produk }} ({{ $produk->total_transaksi }}
                                            transaksi)
                                        </h4>
                                        <table class="min-w-full divide-y divide-gray-200">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    <th class="px-3 py-2 text-left text-xs font-semibold text-gray-600">No.
                                                        Transaksi</th>
                                                ...
                                                @foreach ($produk->detailTransaksi as $detail)
                                                    <tr class="hover:bg-gray-50">
                                                        <td class="px-3 py-2 text-xs">
                                                            <span
                                                                class="font-mono font-semibold text-blue-600">#{{ str_pad($detail->id_penjualan, 6, '0', STR_PAD_LEFT) }}</span>
                                                        </td>
                                                        <td class="px-3 py-2 text-xs text-gray-600">
                                                            {{ \Carbon\Carbon::parse($detail->tanggal_penjualan)->format('d/m/Y H:i') }}
                                                        </td>
                                                        <td class="px-3 py-2 text-xs text-gray-700">
                                                            <i
                                                                class="fas fa-user text-xs mr-1 text-gray-400"></i>{{ $detail->nama_kasir ?? 'Admin' }}
                                                        </td>
                                                        <td class="px-3 py-2 text-xs text-center">
                                                            <span
                                                                class="bg-blue-50 text-blue-700 px-2 py-1 rounded font-semibold">{{ $detail->qty }}
                                                                pcs</span>
                                                        </td>
                                                        <td class="px-3 py-2 text-xs text-right text-gray-600">Rp
                                                            {{ number_format($detail->harga_jual, 0, ',', '.') }}</td>
                                                        <td
                                                            class="px-3 py-2 text-xs text-right font-semibold text-gray-900">
                                            ...
                                            <tfoot class="bg-gray-50">
                                                <tr>
                                                    <td colspan="3" class="px-3 py-2 text-xs font-bold text-gray-700">
                                                        Total</td>
                                                    <td class="px-3 py-2 text-xs text-center">
                                                        <span
                                                            class="bg-blue-100 text-blue-800 px-2 py-1 rounded font-bold">{{ number_format($produk->total_qty, 0, ',', '.') }}
                                                            pcs</span>
                                                    </td>
                                                    <td class="px-3 py-2 text-xs text-right text-gray-600">-</td>
                                                    <td class="px-3 py-2 text-xs text-right font-bold text-indigo-600">Rp
                                                        {{ number_format($produk->total_penjualan, 0, ',', '.') }}</td>
                                                </tr>
                                            </tfoot>
                                        </table>
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-4 py-12 text-center text-gray-500">Tidak ada data produk
                                    terjual</td>
                            </tr>
                        @endforelse
                    </tbody>
                    @if ($detailProduk->count() > 0)
                        <tfoot class="bg-indigo-50 border-t-2 border-indigo-200">
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm text-center">
                                    <span
                                        class="bg-blue-200 text-blue-900 px-3 py-1 rounded-full font-bold">{{ number_format($detailProduk->sum('total_qty'), 0, ',', '.') }}</span>
                                </td>
                                <td class="px-4 py-3 text-sm text-center">
                                    <span
                                        class="bg-green-200 text-green-900 px-3 py-1 rounded-full font-bold">{{ number_format($detailProduk->sum('total_transaksi'), 0, ',', '.') }}</span>
                                </td>
                                <td class="px-4 py-3 text-sm text-right">-</td>
                                <td class="px-4 py-3 text-sm text-right font-bold text-indigo-700">Rp
                                    {{ number_format($detailProduk->sum('total_penjualan'), 0, ',', '.') }}</td>
            ...
            <div class="block lg:hidden space-y-3" id="produkCardList">
                @forelse ($detailProduk as $index => $produk)
                    <div class="produk-card bg-gradient-to-r from-gray-50 to-white border-l-4 border-indigo-500 rounded-lg shadow-sm"
                        data-kategori="{{ $produk->id_produk_kategori ?? '' }}">
                        <div class="p-4">
                            <div class="flex items-start justify-between mb-3">
                                <div class="flex-1 min-w-0 mr-2">
                                    <h3 class="font-bold text-gray-900 text-sm truncate mb-1">{{ $produk->nama_produk }}
                                    </h3>
                                ...
                                <span
                                    class="flex-shrink-0 bg-indigo-100 text-indigo-800 px-2.5 py-1 rounded-full text-xs font-bold">#{{ $index + 1 }}</span>
                            </div>
                            <div class="grid grid-cols-2 gap-2 mb-3">
                                <div class="bg-blue-50 rounded-lg p-2">
                                    <p class="text-xs text-blue-600 font-medium mb-1">Terjual</p>
                                    <p class="text-lg font-bold text-blue-700">
                                        {{ number_format($produk->total_qty, 0, ',', '.') }}</p>
                                </div>
                            ...
                            <div class="space-y-2 pt-3 border-t border-gray-200">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-sm font-semibold text-gray-800">Rp
                                        {{ number_format($produk->harga_satuan, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="text-sm font-medium text-gray-700">{{ number_format($produk->rata_rata_qty, 1) }}
                                        pcs</span>
                                </div>
                            </div>
                            <button onclick="toggleDetailMobile({{ $index }})"
                                class="w-full mt-3 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white rounded-lg text-xs font-semibold transition-all flex items-center justify-center">
                                <i class="fas fa-chevron-down mr-2 transition-transform duration-300"
                                    id="icon-mobile-{{ $index }}"></i>
                        ...
                        <div id="detail-mobile-{{ $index }}"
                            ...
                            <h4 class="text-xs font-bold text-gray-700 mb-2 flex items-center">
                                <i class="fas fa-list-ul mr-2"></i>Detail Transaksi ({{ $produk->total_transaksi }}x)
                            </h4>
                                ...
                                @foreach ($produk->detailTransaksi as $detail)
                                    <div class="bg-white rounded-lg p-2 border border-gray-200">
                                        <div class="flex justify-between items-start mb-1">
                                            <span
                                                class="text-xs font-mono font-semibold text-blue-600">#{{ str_pad($detail->id_penjualan, 6, '0', STR_PAD_LEFT) }}</span>
                                            <span
                                                class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($detail->tanggal_penjualan)->format('d/m/Y H:i') }}</span>
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-xs text-gray-600">Qty: <strong>{{ $detail->qty }}</strong>
                                                pcs</span>
                                            <span class="text-xs font-semibold text-gray-900">Rp
        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6">
                ...
                @forelse ($transaksi as $item)
                    <div class="bg-gradient-to-r from-gray-50 to-white border border-gray-200 rounded-lg p-4">
                        <div class="flex items-center justify-between mb-3">
                            <span
                                class="font-mono text-sm font-bold text-blue-600">#{{ str_pad($item->id_penjualan, 6, '0', STR_PAD_LEFT) }}</span>
            ...
            <div class="hidden lg:block overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($transaksi as $item)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 text-sm font-semibold">
                                    #{{ str_pad($item->id_penjualan, 6, '0', STR_PAD_LEFT) }}</td>
                                <td class="px-4 py-3 text-sm">
                                    {{ \Carbon\Carbon::parse($item->tanggal_penjualan)->format('d/m/Y H:i') }}</td>
                                ...
                                <td class="px-4 py-3 text-sm text-right text-green-600 font-semibold">Rp
                                    {{ number_format($item->kembalian_pembayaran, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-center">
                                    <a href="{{ route('transaksi.struk', $item->id_penjualan) }}" target="_blank"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-8 text-center text-gray-500">Tidak ada transaksi pada
                                    periode ini</td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            {{-- ===== PAGINATION RIWAYAT TRANSAKSI (DIPERBAIKI) ===== --}}
            @if ($transaksi->hasPages())
                <div class="mt-4 pt-4 border-t border-gray-200">
                    {{-- Tambahkan parameter filter ke link paginasi agar tidak hilang saat pindah halaman --}}
                    {{ $transaksi->appends([
                            'tanggal_mulai' => $tanggalMulai,
                            'tanggal_selesai' => $tanggalSelesai,
                            'page_terlaris' => request('page_terlaris', 1),
                            'page_kasir' => request('page_kasir', 1),
                        ])->links() }}
                </div>
            @endif
        </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            // Responsive chart update on resize
            let resizeTimer;
            ...
            function filterProduk() {
                ...
                document.querySelectorAll('.produk-card').forEach(card => {
                    const matchSearch = card.textContent.toLowerCase().includes(searchValue);
                    const matchKategori = !kategoriValue || card.getAttribute('data-kategori') === kategoriValue;
                    card.style.display = (matchSearch && matchKategori) ? '' : 'none';
                });

                // Desktop rows
                document.querySelectorAll('.produk-row').forEach(row => {
                    const matchSearch = row.textContent.toLowerCase().includes(searchValue);
                    const matchKategori = !kategoriValue || row.getAttribute('data-kategori') === kategoriValue;
                    const show = matchSearch && matchKategori;
                    row.style.display = show ? '' : 'none';

                    // Sembunyikan detail row jika baris induk tersembunyi
